      ...
      <Modal open={open} onClose={() => { setOpen(false); setConfirmText(""); }} title="Delete Account">
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="block text-sm font-medium">Type DELETE to confirm</label>
            <input
              type="text"
              value={confirmText}
              onChange={(e) => setConfirmText(e.target.value)}
              className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 border-zinc-300"
              placeholder="DELETE"
            />
          </div>
